...
let LogsController = class LogsController {
    ...
    async create(dto) {
        if (!dto.licensePlate || !dto.vehicleType) {
            throw new common_1.BadRequestException('licensePlate and vehicleType are required');
        }
        const validTypes = ['Car', 'Motorcycle', 'Truck'];
        if (!validTypes.includes(dto.vehicleType)) {
            throw new common_1.BadRequestException(`vehicleType must be one of: ${validTypes.join(', ')}`);
        }
